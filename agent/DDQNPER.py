...
from .prioritized_experience import PrioritizedReplayBuffer
...
class DDQNPERAgent:
    ...
    def learn(self):
        ...
        is_weights = T.tensor(is_weights, dtype=T.float32).to(self.device)
        ...
        td_errors = (target_q_values -
                     current_q_values).abs().detach().cpu().numpy()

        self.memory.update_priorities(indices, td_errors)

        losses = F.mse_loss(current_q_values, target_q_values)
        loss = (losses * is_weights.unsqueeze(1)).mean()
...
def play_DDQNPER(env, model_path):
    agent = DDQNPERAgent(env.observation_space, env.action_space)
