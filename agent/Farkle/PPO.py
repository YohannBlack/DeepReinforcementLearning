...
class PPOMemory:
    def __init__(self, batch_size):
        ...
        self.rewards = []
        self.dones = []
        self.action_masks = []  # Add storage for action masks
        self.batch_size = batch_size

    def store_memory(self, state, action, probs, vals, reward, done, action_mask):
...
class PPOAgent:
    ...
    def choose_action(self, state, action_mask, eval_mode=False):
        ...
        action_mask = T.tensor(action_mask, dtype=T.bool).to(device)

        action_probs, value = self.policy(state, action_mask)
    ...
    def learn(self):
        for _ in range(self.epochs):
            ...
            vals_arr = T.tensor(np.array(self.memory.vals),
                                dtype=T.float32).to(device)
            mask_arr = T.tensor(
                np.array(self.memory.action_masks), dtype=T.bool).to(device)
            ...
            for batch in self.memory.generate_batches():
                ...
                action_masks = mask_arr[batch]

                action_probs, critic_value = self.policy(states, action_masks)
    ...
    def train(self, env, episodes=10000, eval_frequency=1000,
        ...
        for episode in tqdm(range(episodes)):
            ...
            while not done:
                ...
                action, prob, val = self.choose_action(state, valid_actions)
...
def play_farkle_ppo(env, model_path):
