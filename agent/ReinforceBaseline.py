...
class ReinforceBaselineAgent:
    ...
    def train(self, env, episodes: int = 10000, eval_frequency: int = 1000,
        ...
        self._plot_metrics(training_rewards, eval_metrics, policy_losses,
                           value_losses, eval_episodes, save_dir, episodes)
    ...
    def _save_results(self, results, save_dir):
        ...
        model_path = os.path.join('models', *model_dir, 'model.pth')
    ...
    def _plot_metrics(self, training_rewards, eval_metrics, policy_losses,
        ...
        plt.tight_layout()
        plt.savefig(f'{save_dir}/training_plots_{(episodes+1)//1000}k.png')
        plt.close()


def play_reinforce_baseline(env, model_path):
